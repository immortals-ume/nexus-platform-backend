...
import static com.immortals.notification.service.constants.NotificationServiceConstants.DEFAULT_LOCALE;
import static com.immortals.notification.service.constants.NotificationServiceConstants.TEMPLATE_CACHE;
...
public class ManageTemplateUseCaseImpl implements ManageTemplateUseCase {
    ...
    public com.immortals.platform.domain.notifications.domain.NotificationTemplate createTemplate(com.immortals.platform.domain.notifications.domain.NotificationTemplate template) {
        ...
        NotificationTemplate entity = templateMapper.toEntity(template);
    ...
    @Transactional
    @CacheEvict(value = TEMPLATE_CACHE, allEntries = true)
    public com.immortals.platform.domain.notifications.domain.NotificationTemplate updateTemplate(Long id, com.immortals.platform.domain.notifications.domain.NotificationTemplate template) {
    ...
    public com.immortals.platform.domain.notifications.domain.NotificationTemplate getTemplate(Long id) {
        log.debug("Getting template by id: {}", id);
        
        NotificationTemplate entity = templateRepository.findById(id)
            .orElseThrow(() -> new IllegalArgumentException("Template not found: id=" + id));
    ...
    public com.immortals.platform.domain.notifications.domain.NotificationTemplate getTemplate(String templateCode, String locale) {
    ...
    @Override
    @Transactional(readOnly = true)
    public List<com.immortals.platform.domain.notifications.domain.NotificationTemplate> getAllTemplates() {
    ...
    @Override
    @Transactional(readOnly = true)
    public List<com.immortals.platform.domain.notifications.domain.NotificationTemplate> getTemplatesByChannel(String channel) {
        log.debug("Getting templates by channel: {}", channel);
        
        return templateRepository.findByChannel(channel).stream()
            .filter(NotificationTemplate::isUsable)
            .map(templateMapper::toModel)
            .collect(Collectors.toList());
    }
    ...
    public String renderTemplate(String templateCode, String locale, Map<String, Object> variables) {
        ...
        com.immortals.platform.domain.notifications.domain.NotificationTemplate template = getTemplate(templateCode, locale);
