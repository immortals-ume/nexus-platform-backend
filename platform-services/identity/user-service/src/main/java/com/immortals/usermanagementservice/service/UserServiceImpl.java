...
import com.immortals.platform.domain.auth.enums.AddressStatus;
import com.immortals.platform.domain.auth.enums.UserTypes;
...
import com.immortals.platform.common.exception.UserException;
...
public class UserServiceImpl implements UserService {
    ...
    public User getUserByUsername(String username) {
        try {
            // Try declarative cache first, fallback to hash cache from cache-starter
            try {
                return getCachedUserByUsername(username);
            } catch (Exception e) {
                log.debug("Declarative cache miss, trying hash cache for user: {}", username);
                Object cachedUser = hashCacheService.get(USER_HASH_KEY + ":" + username, username);
                if (cachedUser instanceof User) {
                    log.debug("User retrieved from hash cache: [username={}]", username);
